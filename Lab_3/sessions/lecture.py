...
class Lecture(Session):
    ...
    def __init__(self, name: str, period_number: int, room: int, session_date: date, lecturer: Lecturer) -> None:
        ...
        if key in Lecture.OCCUPIED_ROOMS:
            raise ValueError(f"Room {self.room} is already occupied for period {self.period_number}")

        Lecture.OCCUPIED_ROOMS.append(key)

    def per_form(self) -> None:
        start, end = LESSON_TIMES[self.period_number]
        print(f"Lecture '{self.name}' from {start} to {end} in room {self.room}, "
              f"conducted by {self.lecturer.name}")
